...
def render():
    ...
    with tab_list:
        ...
        if filtered.empty:
            st.info("لا يوجد مستخدمون بعد أو لا نتائج مطابقة للبحث.")
        else:
            if can("rbac", "export"):
                st.download_button(
    ...
    with tab_add:
        if not can("rbac", "add"):
            st.warning("🚫 لا تملك صلاحية إضافة مستخدم.")
        else:
            st.markdown("##### 👤 بيانات الحساب")
            username = st.text_input("اسم المستخدم (فريد) *", key="rbac_add_username")
            full_name = st.text_input("الاسم الكامل الثلاثي *", key="rbac_add_fullname")
            ...
            password = st.text_input("كلمة المرور *", type="password", key="rbac_add_password")
            ...
            if st.button("➕ إنشاء الحساب", type="primary", key="rbac_add_submit"):
