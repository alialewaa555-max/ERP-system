...
def render():
    ...
    with tab_list:
        ...
        if filtered.empty:
            st.info("لا توجد نفقات مسجّلة بعد. أضف أول نفقة من تبويب ➕ إضافة نفقة.")
        else:
            if can("expenses", "export"):
                c1, c2 = st.columns(2)
    ...
    with tab_add:
        if not can("expenses", "add"):
            st.warning("🚫 لا تملك صلاحية إضافة نفقة. راجع المالك لتفعيل هذه الصلاحية.")
        else:
            title = st.text_input("عنوان النفقة *", key="exp_add_title")
            amount = st.number_input("التكلفة *", min_value=0.0, step=1000.0, key="exp_add_amount")
            category = st.text_input("التصنيف (مثال: إيجار، كهرباء، رواتب إدارية...)", key="exp_add_category")
            expense_date = st.date_input("تاريخ النفقة", key="exp_add_date")
            notes = st.text_area("ملاحظات", key="exp_add_notes")
            if st.button("➕ إضافة النفقة", type="primary", key="exp_add_submit"):
