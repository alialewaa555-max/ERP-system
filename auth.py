...
try:
    ...
    def _get_cookie_manager():
        if "_cookie_manager_instance" not in st.session_state:
            st.session_state["_cookie_manager_instance"] = stx.CookieManager(key="suleiman_erp_cookie_manager")
...
def logout():
    ...
    st.session_state["session_token"] = None    يدعم كلمتي المرور: المشفرة (sha256) والقديمة النصية الصريحة توافقاً مع
    قواعد البيانات المُنشأة سابقاً. يُنصح بترحيل كل الكلمات إلى SHA-256.
    """
    if stored is None:
        return False
